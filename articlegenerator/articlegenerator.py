...
# Set OpenAI API key
openai.api_key = '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'
...
def upload_to_wordpress(title, content):
    ...
    url = "https://boncanin.fr/wp-json/wp/v2/posts"
    headers = {
        "Authorization": "Bearer NAmfJ5PLX*WB}4}7X8q(W2@<a;Z'DaLGGwPk.{EW]8T:JZdfXU@nOHP$wBm",
    }
    data = {
        "title": title,
        "content": content,
...
def upload_image_to_wordpress(filename):
    url = "https://boncanin.fr/wp-json/wp/v2/media"
    headers = {
        "Authorization": "Bearer NAmfJ5PLX*WB}4}7X8q(W2@<a;Z'DaLGGwPk.{EW]8T:JZdfXU@nOHP$wBm",
...
# Replace this with the appropriate prompt for your blog post
image_prompt = "A patterned repeated background image-only with coffee"
